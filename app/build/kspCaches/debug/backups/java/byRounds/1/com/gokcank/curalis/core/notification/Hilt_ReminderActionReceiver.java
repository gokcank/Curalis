...
public abstract class Hilt_ReminderActionReceiver extends BroadcastReceiver {
  ...
  protected void inject(Context context) {
    if (!injected) {
      synchronized (injectedLock) {
        if (!injected) {
          ((ReminderActionReceiver_GeneratedInjector) BroadcastReceiverComponentManager.generatedComponent(context)).injectReminderActionReceiver(UnsafeCasts.<ReminderActionReceiver>unsafeCast(this));
          injected = true;
        }
      }
    }
